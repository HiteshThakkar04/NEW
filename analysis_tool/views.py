...
def data_insert_db(is_lead_for, upload_file_path, arrhythmia_for):
    data_collection = db['ECG_DATA']
    ...
    # Handle headers that are strings
    if any(str(_).isalpha() for _ in all_lead_data.columns):
        # CSV has proper column names
        ...
        # CSV has numeric columns, rename them
        col_map = {
            "2_Lead": {0: 'II'},
        ...
        available_indices = [i for i, name in col_map[is_lead_for].items() if i in all_lead_data.columns]
    ...
    exists = data_collection.count_documents({
        'Arrhythmia': arrhythmia_for,
        'Lead': is_lead_for,
        'Data': ecg_data_dic['Data']
    }) > 0
...
def check_arrhythmia_model(category, category_path, upload_file_path, is_lead_for, filename):
    ...
    if category == 'afib_afl':
        ...
        if result_dic['is_error'] == False:
            db_msg = data_insert_db(is_lead_for, upload_file_path, result_dic['afib_afl_result'])
        ...
        if result_dic['is_error'] == False:
            db_msg = data_insert_db(is_lead_for, upload_file_path, result_dic['block_label'])
        ...
        if result_dic['is_error'] == False:
            db_msg = data_insert_db(is_lead_for, upload_file_path, result_dic['mi_result'])
        ...
        if result_dic['is_error'] == False:
            db_msg = data_insert_db(is_lead_for, upload_file_path, result_dic['pvc_label'])
        ...
        if result_dic['is_error'] == False:
            db_msg = data_insert_db(is_lead_for, upload_file_path, result_dic['pac_label'])
        ...
        if result_dic['is_error'] == False:
            db_msg = data_insert_db(is_lead_for, upload_file_path, result_dic['jr_label'])        
        ...
        msg = f"Vifib Result: {result_dic['vifib_result']}"
        if result_dic['is_error'] == False:

            db_msg = data_insert_db(is_lead_for, upload_file_path, result_dic['vifib_result'])
