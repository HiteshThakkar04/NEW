...
def check_vfib_vfl_model(low_ecg_signal, let_inf_moedel):
    label = 'Abnormal'
    img_path = r"analysis_tool/ecg_models/vifib_vfl/vflutter_img"
    ...
    while fi < total_data:
        ...
        save_path = os.path.join(img_path, f"p_{fi}.jpg")
    ...
    files = sorted(glob.glob(os.path.join(img_path, "*.jpg")), key=extract_number)
...
def model_check_for_ecg_data(model_path, file_path, is_lead_for, save_result):
    let_inf_moedel = load_tflite_model(model_path) # add your model path
    frequency = 200
    label_counts = {}
    hr_counts={}
    pdf = PDF()
