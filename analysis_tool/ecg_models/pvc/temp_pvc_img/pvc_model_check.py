...
class PVCDetection:
    ...
    def get_pvc_data(self, patientid, date_time, interpreter, input_details, output_details):
        ...
        for lead in all_lead_data.keys():
            if lead in ['II','III', 'I' ]: #['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'v1', 'v2', 'v3', 'v4', 'v5', 'v6']:
                lead_data = {}
                ecg_signal = all_lead_data[lead]
                lis= []
                count= 1
                if self.is_lead == 'lead_2':
                    base_ecg = baseline_construction_200(ecg_signal,131) # 101
                    pvc_data = lowpass_1(base_ecg, cutoff=0.2)
        ...
        if len(all_lead_pvc_data.keys()) > 1:
            ...
            combine_pvcs = [1 if sum(sublist[i] for sublist in all_pvc_list_final) > 1 else 0 for i in range(len(all_pvc_list_final[0]))]
            ...
            combine_lbbbs = [1 if sum(sublist[i] for sublist in all_lbbb_list_final) > 1 else 0 for i in range(len(all_lbbb_list_final[0]))]
            ...
            combine_rbbbs = [1 if sum(sublist[i] for sublist in all_rbbb_list_final) > 1 else 0 for i in range(len(all_rbbb_list_final[0]))]
...
def check_arrhythmia(all_lead_data, date_time, interpreter,temp_pvc_img_path, input_details, output_details):
    fa = 200
...
def plotting(arrhythmia_result ,all_leads_data, save_path, local_name):
    num_leads = len(all_leads_data.keys())
    if num_leads == 1:
        fig = plt.figure(figsize=(10, 5))
    else:
        fig = plt.figure(figsize=(14, 8))
    lead_names = list(all_leads_data.keys())
    
    plt.suptitle(f"{local_name}_{arrhythmia_result['pvc_label']}") # 
    ...
    for idx, lead in enumerate(lead_names):
        # ['I','II', 'III','aVL','aVR','aVF','v5']
        if lead in ['I','II', 'III','aVL','aVR', 'aVF','v1', 'v2', 'v3', 'v4', 'v5', 'v6']:
            all_detected_data = all_leads_data[lead]
            base_ecg = baseline_construction_200(all_detected_data,131)
            lowpass_signal = lowpass_1(base_ecg, cutoff=0.2)
            rpeaks = detect_beats(lowpass_signal, 200)
            pvc_index = arrhythmia_result['pvc_index']
            ax = fig.add_subplot(rows, cols, idx + 1)
            ax.plot(lowpass_signal, label=lead)
            ax.plot(rpeaks, lowpass_signal[rpeaks], 'go', label='rpeak')
            if pvc_index: 
                ax.plot(pvc_index, lowpass_signal[pvc_index], 'ro', label='pvc')
            ax.legend()
    if num_leads == 1:
        analysis_lead = 'II'
    else:
        analysis_lead = 'II, III,I'
    pvc_r_index = ', '.join(map(str, arrhythmia_result['pvc_index']))
    with open(save_path + csv_name, 'a') as csv:
        np.savetxt(csv, np.array([local_name, arrhythmia_result['pvc_label'], analysis_lead, pvc_r_index]).reshape(1,-1), delimiter= ',', fmt = '%s', comments ='')
...
def model_check_for_ecg_data(model_path, file_path, is_lead_for, save_result):
    ...
    for fn in glob.glob(file_path):
        ...
        while i < all_lead_data.shape[0]:
            ...
            arrhythmia_detector = check_arrhythmia(ecg_data, date_time,interpreter,temp_pvc_img_path, input_details, output_details)
            print(arrhythmia_detector,"==========lllll")
            plotting(arrhythmia_detector ,ecg_data, save_result, local_name)
            i += steps
